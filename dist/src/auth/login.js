...
export function parseCallback(callbackUrl, expectedState) {
    const params = new URL(callbackUrl).searchParams;
    if (params.get('state') !== expectedState)
        throw new Error('state mismatch');
    return params;
}
function waitForCallback(provider, serverRef) {
...
export async function runLogin() {
    const provider = new MobbinOAuthProvider((url) => {
        void (async () => {
            try {
                ...
                process.stderr.write(`Opening your browser to authorize mobbin-axi…\n`);
            }
            catch {
                process.stderr.write(`Open this URL in your browser to authorize mobbin-axi:\n${url}\n`);
            }
        })();
    });
    ...
    const params = await Promise.race([
        ...
        new Promise((_, reject) => setTimeout(() => {
            ...
            reject(new Error('Login timed out after 2 minutes — run `mobbin-axi login` again'));
        }, 120_000).unref()),
    ]);
    ...
    await new Client({ name: 'mobbin-axi', version: '0.1.0' }).connect(new StreamableHTTPClientTransport(new URL(MCP_URL), {
        authProvider: provider,
    }));
